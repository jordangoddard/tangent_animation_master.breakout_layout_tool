...
class CheckLayout(object):
    ...
    def open_blendfile_scene(self):
        """
        Open the '.BLEND' file for processing
        """
        with bpy.data.libraries.load(self.filepath) as (data_from, data_to):
             data_to.scenes = [self.seqshot]
        self.scene = bpy.data.scenes[self.seqshot]
        print(self.scene)
    ...
    def write_temporary_check_layout_error_log(self): #jordan                                           # This will log the data to a file
        ...
        selection_name = self.scene_name
        try:
            fileread = open("T:\\Development\\tools\\breakout_layout_tool\\error_logs\\%s_log_%s00.log"%(self.scene_name[:3], date_var), "r")
            fileread.close()
        except:
            file = open("T:\\Development\\tools\\breakout_layout_tool\\error_logs\\%s_log_%s00.log"%(self.scene_name[:3], date_var), "w")
            file.write("------------------%s------------------\n\n\n"%self.scene_name)
            for error in self.error_log:
                file.write("%s: %s\n" %(self.scene_name, error))
            file.close()
        else:
            file = open("T:\\Development\\tools\\breakout_layout_tool\\error_logs\\%s_log_%s00.log"%(self.scene_name[:3], date_var), "a")
            file.write("\n\n\n------------------%s------------------\n\n\n"%self.scene_name)
            for error in self.error_log:
                file.write("%s: %s\n" %(self.scene_name, error))
            file.close()
...
class BreakOut(object):
    ...
    def create_armature_proxies(self, new_group_name): #jordan/wayne
        ...
        selection_pattern = bpy.ops.object.select_pattern
        make_proxy = bpy.ops.object.proxy_make
    ...
    def link_in_groups(self, path, obj_name): #wayne
        ...
        with bpy.data.libraries.load(path, link = True) as (data_from, data_to):
            for group in data_from.groups:
                ...
                if group.startswith('grp.'): #and similarity > 0.7:
                    group_name = group
                    #if LOW, pick LOW
    ...
    def create_empty(self, group_name, path, scene_name = None): #wayne
        ...
        for obj in self.new_scene.objects:
            import re
            search = re.search('Empty', obj.name)
            if search:
                obj.name = group_name# print("link nul to %s" %group_name)
                obj.dupli_type = 'GROUP'
                for group in bpy.data.groups:
                    if group.name == group_name and group.library.filepath == path:
    ...
    def set_new_scene_start_end(self): #jordan/hilaire
        ...
        self.scene[1].frame_start = shot_original_frame_start - (shot_original_frame_start-1) # sets 'start' at Frame 1
        self.scene[1].frame_end = (shot_original_frame_end+1) - (shot_original_frame_start-1)
    ...
    def reset_audio_offsets(self): #hilaire
        ...
        for obj in self.new_scene.sequence_editor.sequences_all:                              # iterate through each audio track
            ...
            shot_audio_file_new_start_frame = shot_audio_file_current_start_frame - (shot_original_frame_start-1)    # sets 'start' at Frame 1
    ...
    def reset_animation_offsets(self): #hilaire
        ...
        self.shot_original_scene = self.scene[0]                                              # need original scene info, should likely be a global
        self.shot_new_scene = self.scene[1]                                                   # need target scene info, should likely be a global
        keyframe_offset = self.shot_original_scene.frame_start - 1
            ...
            try:
                ...
                for fcurve in action.fcurves:                                                 # Keyed actions
                    ...
                    for keyframe_position in fcurve.keyframe_points:
                        keyframe_position.co.x = keyframe_position.co.x - keyframe_offset
                        keyframe_position.handle_right.x = keyframe_position.handle_right.x - keyframe_offset
                        keyframe_position.handle_left.x = keyframe_position.handle_left.x - keyframe_offset
            except: 
    ...
    def check_dupli_group(self): #Wayne
        """
        Check that the linked groups are actually connected to the null object
        """
        for obj in self.new_scene.objects:
            if obj.name.startswith('grp') and obj.type is 'EMPTY':
                obj.dupli_group = bpy.data.groups[obj.name]
